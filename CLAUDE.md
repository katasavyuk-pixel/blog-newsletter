...
- **Fase 2 — Newsletter y captación** ✅ (código, con *guards*). Form real + doble opt-in (Resend), route handlers `/api/{subscribe,confirm,unsubscribe,download}`, plantillas React Email, lead magnets (`/recursos` + descarga firmada), baja 1-clic (RFC 8058), `/gracias`, `/baja`, `/privacidad`. Migración `subscribers`+`resources` escrita. **Pendiente de cuentas reales**: crear proyecto Supabase EU + aplicar `migrations/0001`, cuenta Resend + DNS (DKIM/SPF/DMARC en subdominio), Turnstile. Sin esas env, la app corre en modo previsualización.
